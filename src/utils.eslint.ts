 ...
 */
export function flatConfigFilterRules<T extends Partial<Rules>>(
    rules: T,
    mode: "pick" | "exclude",
    ...
    for (const [key, rule] of entries) {
        if (rule !== void 0) {
            const isEntryRuleInSelection = selection.includes(key);
    ...
    const rulesFiltered = _rulesFiltered as Partial<T>;
